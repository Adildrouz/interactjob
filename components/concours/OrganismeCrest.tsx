...
const SIZES = {
  sm: { box: "w-10 h-10 rounded-[12px] rounded-br-[3px]", icon: 18, star: 7, starPos: "top-1 right-1" },
  md: { box: "w-12 h-12 rounded-[13px] rounded-br-[3px]", icon: 22, star: 8, starPos: "top-1 right-1" },
  lg: { box: "w-16 h-16 rounded-[18px] rounded-br-[4px]", icon: 30, star: 11, starPos: "top-1.5 right-1.5" },
};
...
/**
 * Institution crest — a core recurring brand motif for the v2 direction.
 * A uniform emblem keyed by institution TYPE (ministère navy, université/santé
 * teal, collectivité green, justice burgundy, établissement purple, sécurité
 * deep-navy), rendered as a brand-accented badge with a type icon + the
 * Moroccan five-point star. 100% coverage, zero broken images. If a real logo
 * is registered for the institution it is shown instead. Signature cut corner
 * matches the site-wide shape system.
 */
export default function OrganismeCrest({
  ...
  if (logo) {
    return (
      <span
        className={`${box} ${className} flex-shrink-0 grid place-items-center bg-white border border-navy-100 overflow-hidden`}
        title={name}
      >
