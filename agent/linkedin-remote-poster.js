/**
 * InteractJob â€” LinkedIn Remote Jobs Poster
...
async function main() {
  ...
  for (let i = 0; i < shuffled.length; i++) {
    ...
    if (i < shuffled.length - 1) {
      log(`LinkedIn Remote: pause de ${Math.round(DELAY_MS / 1000)}s avant le prochain postâ€¦`);
...
main().catch(err => {
  log(`LinkedIn Remote: ERREUR FATALE â€” ${err.message}`);
  console.error(err);
  process.exit(1);
});
