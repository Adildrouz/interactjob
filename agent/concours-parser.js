/**
 * Concours de la Fonction Publique â€” Scraper
 * Source: alwadifa-maroc.com
 * Fetches latest concours, translates ARâ†’FR via Claude, saves to data/concours.json
 */
...
export async function fetchConcours() {
  ...
  try {
    ...
    log(`Concours: ERREUR listing â€” ${err.message}`);
  ...
  for (const id of newIds) {
    try {
      ...
      try {
        ...
        log(`  âš  Claude fallback pour concours ${id}: ${err.message}`);
        enrichment = {
