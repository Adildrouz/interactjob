...
async function enrichWithClaude(raw) {
  const prompt = `Tu es un assistant sp�cialis� dans les concours de la fonction publique marocaine.
...
R�ponds UNIQUEMENT avec un JSON valide (pas de markdown) avec ces champs :
{
  "title_fr": "Titre traduit en fran�ais (clair et concis)",
  "organization_fr": "Nom de l'organisation en fran�ais",
  "deadline": "Date limite YYYY-MM-DD ou null si non trouv�e",
  "postes": nombre_entier_ou_null,
  "niveau": "Bac / Bac+2 / Bac+3 / Licence / Master / Ing�nieur / Tous niveaux / null",
  "meta_title": "Titre SEO en fran�ais (max 60 chars)",
  "meta_description": "Description SEO en fran�ais (max 155 chars)",
  "summary_fr": "R�sum� en fran�ais du concours (2-3 phrases claires pour les candidats)"
}`;
...
export async function fetchConcours() {
  log('Concours: d�marrage du scraping (alwadifa-maroc.com)');
  ...
  log(`Concours: ${listingIds.length} trouv�s, ${newIds.length} nouveaux à traiter`);
  ...
  for (const id of newIds) {
    try {
      ...
      log(`  ⚠ Concours ${id} ignor�: ${err.message}`);
      failed++;
    }
  }
  ...
  if (newConcours.length > 0) {
    ...
    log(`Concours: ${enriched} enrichis, ${failed} �chou�s. ${newConcours.length} ajout�s. Total: ${updated.length}`);
